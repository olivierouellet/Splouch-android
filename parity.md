...
| Font scale | partial | Every size on the picker, the sheets and the schedule is `sp` off the Material type scale, so it follows the device's font-size setting. **The board header does not**: it takes its four sizes from the height it has — the screen's in portrait, the app bar's 64dp in landscape — and puts each through `toSp()`, which divides the setting back out. Before that it was fixed `sp`, and by 2.0× the EVENT and HEAT words had eaten the row and squeezed the event name to an ellipsis. **The lane rows are the open part.** They take their size from the height they share (`L-15`, `L-16`), but that size is still `sp` and the setting still multiplies it; what hides it is `L-17`'s auto-shrink absorbing the growth in the name cell, while the club, time, delta and place cells have no such give. So the two halves of the board answer the setting differently, which is a decision rather than a bug — a board is a display and arguably should ignore it, but a low-vision spectator reading lane times is exactly who the setting is for. Not resolved |
