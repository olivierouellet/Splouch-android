...
**Verification status (2026-09-18).** `./gradlew :core:test` passes (92 unit tests over
the socket loop, race clock, frame merge, results grid, strings, filters, the app model,
the seed column, the two palettes, `A-11`'s console gate, `L-23`'s lap count and the
string-snapshot coverage check) and `./gradlew :app:assembleDebug` builds.
